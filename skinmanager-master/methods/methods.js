	...
	return new Promise((res, rej) => {
		...
		(err, result) => {
			...
			result.forEach(element => {
				...
				element.head.size = dealScoreOnEachPart(element.head.size / 9);
				element.upper.size = dealScoreOnEachPart(element.upper.size / 18);
				element.lower.size = dealScoreOnEachPart(element.lower.size / 36);
				element.body.size = dealScoreOnEachPart(element.body.size / 37);
				element.headscore = (0.1 * element.head.size * (element.head.score.erythema + element.head.score.thickness + element.head.score.scale)).toFixed(1);
				element.upperscore = (0.2 * element.upper.size * (element.upper.score.erythema + element.upper.score.thickness + element.upper.score.scale)).toFixed(1);
				element.lowerscore = (0.4 * element.lower.size * (element.lower.score.erythema + element.lower.score.thickness + element.lower.score.scale)).toFixed(1);
				element.bodyscore = (0.3 * element.body.size * (element.body.score.erythema + element.body.score.thickness + element.body.score.scale)).toFixed(1);
